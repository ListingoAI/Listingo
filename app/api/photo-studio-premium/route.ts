...
/** Replicate SDK domyślnie owija https URL w FileOutput (ReadableStream); bez tego łatwo dostać null przy odczycie outputu. */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Bazowa instrukcja którą KAŻDY prompt zawiera
...
6. Ultra realistic result, commercial product photography quality.`
    ...
    // ==============================
    // PRESETS PER PLATFORMA
    // ==============================

    const scenePrompts: Record<string, string> = {
      // === ALLEGRO / AMAZON — czyste białe ===
...
${baseInstruction}`,
      ...
      // === CUSTOM (user pisze swój prompt) ===
      'custom': '', // będzie wzięty z customPrompt
    }

    let prompt: string

    if (scene === "custom" && customPrompt) {
      // User napisał własny prompt — dodaj bazowe instrukcje
      prompt = `${productHint}\n${customPrompt}\n${baseInstruction}`
    } else {
    ...
    console.log('Using Flux Kontext Pro for scene:', scene)
    console.log('Prompt:', prompt.substring(0, 100) + '...')

    // ============================================
    // FLUX KONTEXT PRO - edytuje zdjęcie zachowując produkt
    // ============================================

    let output: unknown

    try {
      // safety_tolerance: max 2 when input_image is set (Replicate / BFL schema)
      ...
      console.log('Flux Kontext Pro output:', typeof output)
      ...
      // FALLBACK 1: Flux Kontext Dev (tańszy)
      try {
        console.log('Trying Flux Kontext Dev...')
        output = await replicate.run(
        ...
        // FALLBACK 2: DALL-E 3 (bez edycji, nowe zdjęcie)
        console.log('Fallback to DALL-E 3...')
    ...
    if (!imageUrl) {
      ...
      try {
        ...
        preview = String(output).substring(0, 300)
      }
      console.error('No image URL. Output preview:', preview)
      throw new Error('Model nie zwrócił obrazu')
    }

    // Zwracamy bezpośredni URL (Replicate delivery) — unikamy ogromnego JSON z base64 i limitów odpowiedzi.
    console.log('Success! result URL prefix:', imageUrl.slice(0, 80))
...
// DALL-E fallback (generuje podobne zdjęcie ale bez inputu)
async function generateFallbackWithDALLE(scenePrompt: string, productDesc?: string): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({ error: 'Wszystkie modele zawiodły. Spróbuj ponownie.' }, { status: 500 })
  }
}
