...
function ShoperWordmark() {
  return (
    <svg viewBox="0 0 72 20" xmlns="http://www.w3.org/2000/svg" aria-hidden className="h-6 w-auto shrink-0">
      <text
        x="0"
        y="16"
        fill="currentColor"
...
const LOGO_ENTRIES: LogoEntry[] = [
  ...
  },
  {
    ...
    innerClass: "origin-center scale-[1.15]",
    node: <BrandSvg icon={siEbay} />,
  },
]
...
export function IntegrationLogosMarquee() {
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      >
        {LOGO_ENTRIES.map(({ id, hoverColor, innerClass, node }) => (
          <div
            key={id}
            className={cn(
              "flex min-h-8 min-w-11 items-center justify-center opacity-45 transition-opacity duration-200 hover:opacity-100",
              innerClass
            )}
      ...
      <p className="text-[11px] font-medium tracking-wide text-muted-foreground/40">
